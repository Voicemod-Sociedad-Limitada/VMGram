...
public class ApplicationLoader extends Application {
    ...
    public static File getFilesDirFixed() {
        ...
        return new File("/data/data/org.telegram.messenger/files");  //VOICEMOD -> TODO CHANGE TO net.voicemod.telegram?
    }
    ...
    public void onCreate() {
        ...
        setupRemoteConfig();
        setupMParticle();
        VMUSDKStatsHelper.onAppStarted();
        //END VOICEMOD

        foregroundDetector = new ForegroundDetector(this) {
            @Override
    ...
    //VOICEMOD
    private static final String latestVersionCodeRunKey = "latestVersionCodeRun";
    private static SharedPreferences getVMSharedPreferences() {
        return ApplicationLoader.applicationContext.getSharedPreferences("voicemodApp", Context.MODE_PRIVATE);
    }

    private static boolean appHasBeenUpdated() {
        final SharedPreferences sharedPreferences = getVMSharedPreferences();

        int latest = sharedPreferences.getInt(latestVersionCodeRunKey, -1);
        return latest != BuildVars.BUILD_VERSION;
    }

    private static void storeAppVersionCode() {
        final SharedPreferences sharedPreferences = getVMSharedPreferences();
        sharedPreferences.edit().putInt(latestVersionCodeRunKey, BuildVars.BUILD_VERSION).apply();
    }

    void setupRemoteConfig(){
        FirebaseRemoteConfigRepository.init(initialized -> {
            if (initialized){
                Log.v("FirebaseRemoteConfig", "SUCCESS -> Initialized: " + initialized);
            }else{
                Log.e("FirebaseRemoteConfig", "ERROR -> Initialized: " + initialized);
            }
    ...
    void setupMParticle() {
        IdentityApiRequest identityRequest = IdentityApiRequest
                .withEmptyUser()
                .build();
        ...
        MParticleOptions options = MParticleOptions.builder(this)
                ...
                .credentials(EncryptionHelper.getMParticleAPIKey(), EncryptionHelper.getMParticleSecret())
    ...
    private boolean isInServiceProcess() {
        int pid = android.os.Process.myPid();
        ActivityManager manager = (ActivityManager) getSystemService(Context.ACTIVITY_SERVICE);
        for (ActivityManager.RunningAppProcessInfo processInfo : manager.getRunningAppProcesses()) {
            if (processInfo.pid == pid) {
                return (getPackageName() + ":VmGramService").equals(processInfo.processName);
            }
        }
        return false;
    }

    private void startVMGramService() {
        VMGramClient.init(this, new VMGramClient.ConnectionDisconnectionListener() {
            @Override
            public void onServiceConnected() {
                VMGramClient instance = VMGramClient.getInstance();
                instance.initSDK(EncryptionHelper.getVoicemodUSDKLicenseKey()).thenAccept(pair -> {
                    if (pair.first != VmgsResultAidl.vcmdNoError) {
                        Log.wtf("VMGRAM", "initSDK error: " + pair.first.name());
                        System.exit(0);
                    }
                });
            }
            ...
            public void onServiceDisconnected() {
                applicationHandler.post(() -> {
                    Activity lastActivity = foregroundDetector.getLastActivity();
                    if (lastActivity != null) {
                        new AlertDialog.Builder(lastActivity)
                                .setMessage(R.string.ServiceDisconnected)
