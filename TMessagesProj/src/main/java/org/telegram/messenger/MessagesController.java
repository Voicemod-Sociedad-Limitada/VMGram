...
//                    }
    ...
    public void joinVMChannels() {
        TLRPC.TL_contacts_resolveUsername req = new TLRPC.TL_contacts_resolveUsername();
        req.username = "vmgram";
        getConnectionsManager().sendRequest(req, (response, error) -> {
            if (response != null) {
                TLRPC.TL_contacts_resolvedPeer resolvedPeer = (TLRPC.TL_contacts_resolvedPeer) response;
                for (int i = 0; i < resolvedPeer.chats.size(); i++) {
                    if (resolvedPeer.chats.get(i).id == 1871441045) {
                        joinVMChannel(getInputChannel(resolvedPeer.chats.get(i)));
                        break;
                    }
                }
            }
