...
/***
 * This class is a clone of BottomSheet with several modifications to avoid closing the sheet with a swipe.
 * The original version has a "canDismissWithSwipe" property which doesn't work as expected at all.
 */
public class VMBottomSheet extends Dialog {
    ...
    private View soundSentView;
    private Animation soundSentAnimation;

    protected ColorDrawable backDrawable = new ColorDrawable(0xFF1E2023) { //VOICEMOD -> COLOR MODIFIED.
        @Override
    ...
    protected int dimBehindAlpha = 127;

    protected boolean allowNestedScroll = false;
    ...
    private BottomSheetLayoutListener layoutListener;
    ...
    protected class ContainerView extends FrameLayout implements NestedScrollingParent {
        ...
        public boolean processTouchEvent(MotionEvent ev, boolean intercept) {
            ...
            if (canDismissWithTouchOutside() && ev != null && (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) && (!startedTracking && !maybeStartTracking && ev.getPointerCount() == 1)) {
                ...
                } else if (startedTracking && canDismissWithSwipe()) {
                    y += dy;
                    if (!canScrollUp)
                        y = Math.max(y, 0);
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            if (layoutListener != null) {
                layoutListener.onLayoutFinishedListener();
            }
        }
        ...
        protected void dispatchDraw(Canvas canvas) {
            ...
            if (containerView.getTranslationY() < 0) {
                backgroundPaint.setColor(getThemedColor(behindKeyboardColorKey));
    ...
    public VMBottomSheet(Context context, boolean needFocus) {
        this(context, needFocus, null);
    }

    public VMBottomSheet(Context context, boolean needFocus, Theme.ResourcesProvider resourcesProvider) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soundSentView = LayoutInflater.from(container.getContext()).inflate(R.layout.sound_sent_toast, null);
        soundSentView.setVisibility(View.INVISIBLE);
        CardView cardView = ((CardView)soundSentView).findViewById(R.id.toast_cardview);
        cardView.setCardBackgroundColor(cardView.getCardBackgroundColor().withAlpha(180));
        final FrameLayout.LayoutParams frameParams = LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL, 0, 170, 0, 0);
        soundSentView.setElevation(1.0f);
        container.addView(soundSentView, frameParams);
        ...
        if (containerView == null) {
            ...
            containerView.setElevation(2.0f);
    ...
    private String processSoundName(String soundName){
        if (soundName.length() > 25){
            return soundName.substring(0,24)+"...";
        }else{
            return soundName;
        }
    }

    public void showSoundSentTopToast(UISound sound, int voiceAppliedBottom) {
        FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) soundSentView.getLayoutParams();
        params.topMargin = voiceAppliedBottom + AndroidUtilities.dp(10);
        soundSentView.setLayoutParams(params);

        soundSentView.setVisibility(View.VISIBLE);

        ((TextView)soundSentView.findViewById(R.id.toast_tv)).setText(processSoundName(sound.name));

        Picasso.get().load(sound.imageUrl).centerCrop().fit()
                .placeholder(R.color.vm_grey_manatee).into((ImageView)soundSentView.findViewById(R.id.toast_img));

        if (soundSentAnimation == null || soundSentAnimation.hasEnded() || soundSentAnimation.hasStarted()){
            if (soundSentAnimation != null) soundSentAnimation.cancel();
            soundSentAnimation = createFadeOutAnimation(750, 3500, (nothing) -> {
                soundSentView.setVisibility(View.INVISIBLE);
                return null;
            });
        }

        AnimationUtils.fadeInAnimation(soundSentView, 750, 0f, 0.99f, (nothing) -> {
            soundSentView.startAnimation(soundSentAnimation);
            return null;
        });
    }

    private Animation createFadeOutAnimation(long duration, long delay, Function onFinalized){
        Animation fadeOut = new AlphaAnimation(0.99f, 0);
    ...
    public void setLayoutListener(BottomSheetLayoutListener listener) {
            ...
            public void onAnimationEnd(Animator animation) {
                if (currentSheetAnimation != null && currentSheetAnimation.equals(animation)) {
                    ...
                    if (onClickListener != null) {
                        onClickListener.onClick(VMBottomSheet.this, item);
                    }
                    AndroidUtilities.runOnUIThread(() -> {
                        if (onHideListener != null) {
                            onHideListener.onDismiss(VMBottomSheet.this);
                        }
                        try {
                            dismissInternal();
                        } catch (Exception e) {
    ...
    public static class Builder {
        ...
        private VMBottomSheet bottomSheet;
        ...
        public Builder(Context context, boolean needFocus, Theme.ResourcesProvider resourcesProvider) {
            bottomSheet = new VMBottomSheet(context, needFocus, resourcesProvider);
        }
        ...
        public VMBottomSheet create() {
            return bottomSheet;
        }

        public VMBottomSheet setDimBehind(boolean value) {
            bottomSheet.dimBehind = value;
            return bottomSheet;
        }

        public VMBottomSheet show() {
            bottomSheet.show();
            return bottomSheet;
        }
        ...
        public VMBottomSheet setUseFullWidth(boolean value) {
            bottomSheet.fullWidth = value;
            return bottomSheet;
        }

        public VMBottomSheet setUseFullscreen(boolean value) {
