...
import org.telegram.vm.VMVoIPFragmentHelper;
import org.telegram.vm.VMVoIPToggleButton;
...
public class VoIPFragment implements VoIPService.StateListener, NotificationCenter.NotificationCenterDelegate {
    ...
    VMVoIPToggleButton[] bottomButtons = new VMVoIPToggleButton[4];
    //VOICEMOD
    private View voiceAppliedView, voiceAppliedTopView;
    private VMVoIPToggleButton voiceIdentityButton;
    private LinearLayout linearLayoutButtons;
    private final VMVoIPFragmentHelper vmVoIPFragmentHelper = new VMVoIPFragmentHelper();

    private float voiceToastsDistance = 0;
    //END VOICEMOD
    ...
    Observer<VMVoice> voiceChangedObserver = (Observer<VMVoice>) this::onVoiceChanged;
    ...
    public static void show(Activity activity, boolean overlay, int account) {
        ...
        VMGramLiveData.callSelectedVoice.observeForever(instance.voiceChangedObserver);
    ...
    private void onVoiceChanged(VMVoice voice){
        if (voiceAppliedView != null && voice != null) {
            vmVoIPFragmentHelper.showVoiceAppliedTopToast(activity, voiceAppliedTopView, voiceAppliedView, voice.id);
            VMUSDKStatsHelper.onCallVoiceChanged(voice.name);
        }
    }
    ...
    public static void clearInstance() {
        if (instance != null) {
            ...
            VMGramLiveData.callSelectedVoice.removeObserver(instance.voiceChangedObserver);
            instance.callingUserTextureView.renderer.release();
            instance.currentUserTextureView.renderer.release();
            instance.callingUserMiniTextureRenderer.release();
            instance.destroy();
    ...
    private void setInsets(WindowInsets windowInsets) {
        ...
        ((FrameLayout.LayoutParams) linearLayoutButtons.getLayoutParams()).bottomMargin = lastInsets.getSystemWindowInsetBottom();
        ...
        ((FrameLayout.LayoutParams) voiceAppliedTopView.getLayoutParams()).topMargin = AndroidUtilities.dp(17) + lastInsets.getSystemWindowInsetTop();
    ...
    private void destroy() {
        ...
        if (vmVoIPFragmentHelper.soundBottomSheet != null) {
            vmVoIPFragmentHelper.soundBottomSheet.dismiss();
        }
    }
    ...
    public View createView(Context context) {
        ...
        //VOICEMOD
        LinearLayout topToastLayout = new LinearLayout(context);
        topToastLayout.setOrientation(LinearLayout.VERTICAL);

        voiceAppliedTopView = LayoutInflater.from(activity).inflate(R.layout.voice_applied_toast, null);
        voiceAppliedTopView.setVisibility(View.INVISIBLE);
        //topToastLayout.addView(voiceAppliedTopView, LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL, 0, 6, 0, 6));


        voiceAppliedView = LayoutInflater.from(activity).inflate(R.layout.voice_applied_toast, null);
        voiceAppliedView.setId(R.id.vm_voice_applied_toast);
        voiceAppliedView.setVisibility(View.INVISIBLE);
        //statusLayout.addView(voiceAppliedView, LayoutHelper.createLinear(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL, 0, 6, 0, 6));
        //END VOICEMOD
        ...
        frameLayout.addView(voiceAppliedTopView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL, 0, 17, 0, 0));
        frameLayout.addView(voiceAppliedView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL, 0, 170, 0, 0));

        frameLayout.getViewTreeObserver().addOnGlobalLayoutListener(
                () ->{
                    setMargins(voiceAppliedView, 0, statusLayout.getBottom() + 10, 0, 0);
                    voiceToastsDistance = voiceAppliedView.getY() - voiceAppliedTopView.getY();
                });

        buttonsLayout = new VoIPButtonsLayout(context);
        for (int i = 0; i < 3; i++) {
            bottomButtons[i] = new VMVoIPToggleButton(context);
        }
        ...
        //VOICEMOD
        bottomButtons[3] = new VMVoIPToggleButton(context); //end call button
        voiceIdentityButton = vmVoIPFragmentHelper.createVoiceIdentityButton(context, activity, fragmentView);
        linearLayoutButtons = vmVoIPFragmentHelper.createLinearLayoutButtons(context, voiceIdentityButton, bottomButtons);
        frameLayout.addView(linearLayoutButtons, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT, Gravity.BOTTOM));
        //END VOICEMOD
    ...
    private void showUi(boolean show) {
        ...
        if (!show && uiVisible) {
            ...
            linearLayoutButtons.animate().alpha(0).translationY(AndroidUtilities.dp(50)).setDuration(150).setInterpolator(CubicBezierInterpolator.DEFAULT).start();
            ...
            linearLayoutButtons.setEnabled(false);
            voiceAppliedView.animate().translationY(-voiceToastsDistance).start();
            //VOICEMOD
            ...
            linearLayoutButtons.animate().alpha(1f).translationY(0).setDuration(150).setInterpolator(CubicBezierInterpolator.DEFAULT).start();
            ...
            linearLayoutButtons.setEnabled(true);
            voiceAppliedView.animate().translationY(voiceToastsDistance).start();
            //VOICEMOD
    ...
    private void updateButtons(boolean animated) {
        ...
        if (currentState == VoIPService.STATE_WAITING_INCOMING || currentState == VoIPService.STATE_BUSY) {
            ...
            //VOICEMOD
            vmVoIPFragmentHelper.setDataVoiceIdentityButton(animated, voiceIdentityButton);
            //END VOICEMOD
        }
    ...
    private void setMicrohoneAction(VMVoIPToggleButton bottomButton, VoIPService service, boolean animated) {
    ...
    private void setVideoAction(VMVoIPToggleButton bottomButton, VoIPService service, boolean animated) {
    ...
    private void setSpeakerPhoneAction(VMVoIPToggleButton bottomButton, VoIPService service, boolean animated) {
        if (service.isBluetoothOn()) {
    ...
    private void setFrontalCameraAction(VMVoIPToggleButton bottomButton, VoIPService service, boolean animated) {
        if (!currentUserIsVideo) {
